...
def parse_args():
    parser = argparse.ArgumentParser(prog="facility")
...
def run_file(path: str, solver):
    ...
    instance = FacilityInstance.from_file(path)
    solution = solver.solve(instance)
    elapsed = time.perf_counter() - t0

    valid = solution.verify(instance)

    print(f"{solution.objective:.4f}")
    print(" ".join(str(int(i)) for i in solution.assignment))
